...
-- Nút hình tròn, đặt dưới avatar Roblox (góc trái, y=62 để dưới avt ~62px)
local ToggleBtn = Instance.new("ImageButton")
ToggleBtn.Name = "UIToggleBtn"
ToggleBtn.Size = UDim2.new(0, 50, 0, 50)
ToggleBtn.Position = UDim2.new(0, 5, 0, 62)
...
-- Click chỉ toggle khi không drag
ToggleBtn.MouseButton1Click:Connect(function()
    if dragging then return end
    UIVisible = not UIVisible
    pcall(function() vu32:SetVisible(UIVisible) end)
    pcall(function()
        for _, g in pairs(game:GetService("CoreGui"):GetChildren()) do
            if g:IsA("ScreenGui") and g.Name ~= "UIToggleGui" and g.Name ~= "RobloxGui" then
                g.Enabled = UIVisible
            end
        end
    end)
    BtnStroke.Color = UIVisible and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(255, 80, 80)
end)
